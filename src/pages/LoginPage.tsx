  ...
  return (
    <div className="login-page">
      <div className="login-card page-enter">
        ...
        <h1 className="login-title">Social Video AI</h1>
        <p className="login-sub">Crea videos con IA en segundos</p>

        <button className="login-google-btn" onClick={handleLogin} disabled={loading}>
          {loading ? (
            <><div className="spinner" /> Autenticando...</>
          ) : (
            <><GoogleIcon /> Continuar con Google</>
          )}
        </button>

        <p className="login-footer">Solo para uso académico · UCC 2025</p>
      </div>
    </div>
  );
}
